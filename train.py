...
def main():
	...
	am_smt_w_var = tf.compat.v1.placeholder(tf.float32, ())
	reproj_w_var = tf.compat.v1.placeholder(tf.float32, ())
	...
	# train_step = tf.contrib.layers.optimize_loss(loss, optimizer=tf.compat.v1.train.AdamOptimizer(learning_rate=.05, epsilon=1e-1), learning_rate=None, global_step=global_step)
	train_step = layers.optimize_loss(loss,optimizer=tf.compat.v1.train.AdamOptimizer(learning_rate=.05, epsilon=1e-1),learning_rate=None,global_step=global_step)

	# define saver for saving and restoring
	irn_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope='IRN') if args.train_mode == 'self-train' else tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope='pre_train_IRN')
	...
	if args.train_mode == 'self-train':
		# load am_sup net
		preTrain_irn_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope='pre_train_IRN')
		saver_loadOldVar = tf.compat.v1.train.Saver(preTrain_irn_vars)
		...
		with tf.compat.v1.variable_scope(tf.compat.v1.get_variable_scope(), reuse=tf.compat.v1.AUTO_REUSE):
			...
			vars = tf.train.list_variables('pre_train_model')
			for var_name, _ in vars:
				var = tf.train.load_variable('pre_train_model', var_name)
				new_var_name = var_name.replace('pre_train_IRN', 'IRN')

				new_var = tf.compat.v1.get_variable(name=new_var_name)
	...
	sess.run(trainData_init_op.initializer)
