...
def map_reproj(dm1,map2,cam1,cam2,scale_x1,scale_x2,scale_y1,scale_y2):
	batch_size = tf.shape(input=dm1)[0]
	...
	# u and v are scalars
	u1 = tf.shape(input=dm1)[1]
	v1 = tf.shape(input=dm1)[2]

	# convert u1 and v1 to float, convenient for computation
	u1 = tf.cast(u1, dtype=tf.float32)
	...
	homo_pc_c2 = tf.matmul(W_C_trans2, tf.matmul(tf.linalg.inv(W_C_trans1), homo_pc_c1))

	### project point cloud to cam2 pixel coordinates
	# u in vertical and v in horizontal
	u2 = tf.shape(input=map2)[1]
	v2 = tf.shape(input=map2)[2]

	# convert u2 and v2 to float
	u2 = tf.cast(u2, dtype=tf.float32)
	...
	homo_uv2 = tf.reduce_sum(input_tensor=k2*homo_pc_c2,axis=2)
	...
	u_valid = tf.logical_and(tf.logical_and(tf.logical_not(tf.math.is_nan(u_reproj)), u_reproj>0), u_reproj<u2-1)
	v_valid = tf.logical_and(tf.logical_and(tf.logical_not(tf.math.is_nan(v_reproj)), v_reproj>0), v_reproj<v2-1)
	...
	img_inds = tf.tile(tf.expand_dims(tf.cast(tf.range(batch_size), dtype=tf.float32), axis=1), multiples=[1,num_pix])
	request_points1 = tf.stack([tf.boolean_mask(tensor=img_inds,mask=pixels), tf.boolean_mask(tensor=v_reproj,mask=pixels), tf.boolean_mask(tensor=u_reproj,mask=pixels)], axis=1)
	...
	ndims = tf.shape(input=map2)[3]
	shape = [batch_size, tf.cast(u1, dtype=tf.int32), tf.cast(v1, dtype=tf.int32),3]

	pixels = tf.reshape(pixels,shape=tf.stack([batch_size, tf.cast(u1, dtype=tf.int32), tf.cast(v1, dtype=tf.int32)],axis=0))
	indices = tf.cast(tf.compat.v1.where(tf.equal(pixels,True)), dtype=tf.int32)
...
def interpImg(unknown,data):
	...
	img_inds = tf.cast(unknown[:,0], dtype=tf.int32)
	x = unknown[:,1]
	y = unknown[:,2]
	# rgb_inds = tf.to_int32(unknown[:,3])

	low_x = tf.cast(tf.floor(x), dtype=tf.int32)
	high_x = tf.cast(tf.math.ceil(x), dtype=tf.int32)
	low_y = tf.cast(tf.floor(y), dtype=tf.int32)
	high_y = tf.cast(tf.math.ceil(y), dtype=tf.int32)

	# measure the weights for neighbourhood average based on distance
	dist_low_x = tf.expand_dims(x - tf.cast(low_x, dtype=tf.float32), axis=-1)
	dist_high_x = tf.expand_dims(tf.cast(high_x, dtype=tf.float32) - x, axis=-1)
	dist_low_y = tf.expand_dims(y - tf.cast(low_y, dtype=tf.float32), axis=-1)
	dist_high_y = tf.expand_dims(tf.cast(high_y, dtype=tf.float32) - y, axis=-1)
