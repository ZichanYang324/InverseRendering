...
def md_read_func(filename):
	...
	input, dm, nm, cam, scaleX, scaleY, mask = tf.compat.v1.py_func(_read_pk_function, [filename], [tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32])	
...
def md_construct_inputPipeline(items, batch_size, flag_shuffle=True):
	...
	if flag_shuffle:
		data = data.apply(tf.data.experimental.shuffle_and_repeat(buffer_size=100000))
	else:
		data = data.repeat()
	data = data.apply(tf.data.experimental.parallel_interleave(md_read_func, cycle_length=batch_size, block_length=1, sloppy=False ))
