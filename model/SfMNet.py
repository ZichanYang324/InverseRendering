...
def SfMNet(inputs, height, width, name='', n_layers=12, n_pools=2, is_training=True, depth_base=64):
	...
	for i,f_in,f_out in zip(range(1,conv_layers+2),f_in_conv,f_out_conv):
		...
		if np.mod(i-1,nlayers_befPool)==0 and i<=n_pools*nlayers_befPool+1 and i != 1:
			...
			conv_out = layers.conv2d(conv_out,num_outputs=f_out,kernel_size=[3,3],stride=[1,1],padding='SAME',normalizer_fn=layers.batch_norm, normalizer_params=batch_norm_params,weights_initializer=tf.compat.v1.random_normal_initializer(mean=0,stddev=np.sqrt(2/9/f_in)),weights_regularizer=tf.keras.regularizers.L2(l2=1e-5),biases_initializer=None,scope=scope, trainable=is_training)
			conv_out = tf.nn.max_pool2d(input=conv_out, ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME')
	...
	for i,f_in,f_out in zip(range(1,deconv_layers+1),f_in_deconv,f_out_amDeconv):
		...
		if np.mod(i,nlayers_befPool)==0 and i<=n_pools*nlayers_befPool:
			with tf.compat.v1.variable_scope(scope):
				# W = tf.compat.v1.get_variable(regularizer=layers.l2_regularizer(scale=1e-5),initializer=get_bilinear_filter([3,3,f_out,f_in],2),shape=[3,3,f_out,f_in],name='filter', trainable=is_training)	
				W = tf.compat.v1.get_variable(regularizer=tf.keras.regularizers.L2(l2=1e-5),initializer=get_bilinear_filter([3,3,f_out,f_in],2),shape=[3,3,f_out,f_in],name='filter', trainable=is_training)
				...
				output_shape = tf.shape(input=tmp)
				am_deconv_out = tf.nn.conv2d_transpose(am_deconv_out,filters=W,output_shape=output_shape,strides=[1,2,2,1],padding='SAME')
				am_deconv_out = layers.batch_norm(scope=scope,activation_fn=tf.nn.relu,inputs=am_deconv_out,decay=0.9, center=True, scale=True, param_initializers={'beta_initializer':tf.compat.v1.zeros_initializer(),'gamma_initializer':tf.compat.v1.ones_initializer(),'moving_variance_initializer':tf.compat.v1.ones_initializer(),'moving_average_initializer':tf.compat.v1.zeros_initializer()}, param_regularizers={'beta_regularizer':None,'gamma_regularizer':tf.keras.regularizers.L2(l2=1e-5)},is_training=is_training,trainable=is_training)


			# tmp = layers.conv2d(tmp,num_outputs=f_out,kernel_size=[3,3],stride=[1,1],padding='SAME',normalizer_fn=layers.batch_norm, normalizer_params=batch_norm_params,weights_initializer=tf.compat.v1.random_normal_initializer(mean=0,stddev=np.sqrt(2/9/f_in)),weights_regularizer=layers.l2_regularizer(scale=1e-5),biases_initializer=None,scope=scope,trainable=is_training)
			tmp = layers.conv2d(tmp,num_outputs=f_out,kernel_size=[3,3],stride=[1,1],padding='SAME',normalizer_fn=layers.batch_norm, normalizer_params=batch_norm_params,weights_initializer=tf.compat.v1.random_normal_initializer(mean=0,stddev=np.sqrt(2/9/f_in)),weights_regularizer=tf.keras.regularizers.L2(l2=1e-5),biases_initializer=None,scope=scope,trainable=is_training)
			am_deconv_out = tmp + am_deconv_out


		elif i==deconv_layers:
			am_deconv_out = layers.conv2d(am_deconv_out,num_outputs=f_out,kernel_size=[3,3],stride=[1,1],padding='SAME',normalizer_fn=None,activation_fn=None,weights_initializer=tf.compat.v1.random_normal_initializer(mean=0,stddev=np.sqrt(2/9/f_in)),weights_regularizer=tf.keras.regularizers.L2(l2=1e-5),scope=scope,trainable=is_training)


		else:
			am_deconv_out = layers.conv2d(am_deconv_out,num_outputs=f_out,kernel_size=[3,3],stride=[1,1],padding='SAME',normalizer_fn=layers.batch_norm, normalizer_params=batch_norm_params,weights_initializer=tf.compat.v1.random_normal_initializer(mean=0,stddev=np.sqrt(2/9/f_in)),weights_regularizer=tf.keras.regularizers.L2(l2=1e-5),biases_initializer=None,scope=scope,trainable=is_training)
	...
	for i,f_in,f_out in zip(range(1,deconv_layers+1),f_in_deconv,f_out_nmDeconv):
		...
		if np.mod(i,nlayers_befPool)==0 and i<=n_pools*nlayers_befPool:
			with tf.compat.v1.variable_scope(scope):
				...
				tmp = conv_out_list[-np.int32(i/nlayers_befPool)]
				output_shape = tf.shape(input=tmp)
				nm_deconv_out = tf.nn.conv2d_transpose(nm_deconv_out,filters=W,output_shape=output_shape,strides=[1,2,2,1],padding='SAME')
			...
			nm_deconv_out = layers.conv2d(nm_deconv_out,num_outputs=f_out,kernel_size=[3,3],stride=[1,1],padding='SAME',normalizer_fn=layers.batch_norm, normalizer_params=batch_norm_params,weights_initializer=tf.compat.v1.random_normal_initializer(mean=0,stddev=np.sqrt(2/9/f_in)),weights_regularizer=tf.keras.regularizers.L2(l2=1e-5),biases_initializer=None,scope=scope,trainable=is_training)
...
def get_bilinear_filter(filter_shape, upscale_factor):
    ...
    return tf.compat.v1.constant_initializer(weights)
