...
def illuDecomp(input, am, nm, gamma):
	...
	# find defined pixels
	mask = tf.not_equal(tf.reduce_sum(input_tensor=nm,axis=-1),0)
	num_iter = tf.shape(input=mask)[0]
	...
	def body(i, output):
		...
		nm_ = nm[i]
		shadings_pixel = tf.boolean_mask(tensor=shadings_, mask=mask_)
		nm_ = tf.boolean_mask(tensor=nm_, mask=mask_)

		# E(n) = A*L_SHcoeffs
		total_npix = tf.shape(input=nm_)[0:1]
	...
	_, output = tf.while_loop(cond=condition, body=body, loop_vars=[i,output])
